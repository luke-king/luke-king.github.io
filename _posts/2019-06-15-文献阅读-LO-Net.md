...
- Mapping: scan-to-map refinement（**本部分需要更多时间去了解细节，目前有些部分还是没搞懂**）
  ...
  - **$\bf{\Psi}$**：通过用$T_t$的线性插值对自运动进行补偿来消除由$S_t$得到的点云运动畸变，然后使用$M_{init}$将纠正的$S_t$变换到全局坐标系中，准备匹配。
    ...
    假设 $\bf{p}_i = (p_{i_x}, p_{i_y}, p_{i_z},1)^T \textrm{是} S_t$ 中的一个点，$$\bf{m}_i = (m_{i_x}, m_{i_y}, m_{i_z}, 1)^T$$ 是由之前t-1个时间点建立起来图中的某个对应点，$$\bf{n}_i = (n_{i_x}, n_{i_y}, n_{i_z}, 0)^T$$ 是 $$\bf{m}_i$$ 的单位法向量。mapping的目标是找到一个最优的刚体变换：
    $$
